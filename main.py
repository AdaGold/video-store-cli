...
# 
def make_choice(options, video, customer, choice=None):
...
def main(play=True):
    ...
    while play:
        ...
        # Options 8: Check in a video
        elif choice == '8':
            customer_id = input("Please provide the ID of the customer who is returning a video ")
            video_id = input("Please provide the ID of the video that is being returned ")
            response = rental.check_in(int(customer_id), int(video_id))
            
            if response.status_code != 200:
                print("The data provided was not sufficient, and the transaction is being cancelled")
            else:
                print("Thank you for returning your rental. Have a great day!")

        # Option 9: List all options
        elif choice == '9':
            list_options()
