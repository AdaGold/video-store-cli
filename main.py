...
def video_menu_options(subchoice, video_store):
    ...
    if subchoice=='1':
        ...
        subplay=False
        print_spaces()
        print("== Thank you so much for visiting our VIDEO information. We hope to see you soon! ==\n")
        print_spaces()
        print_new_line()
        list_models()
...
def rentals_menu_options(subchoice, video_store):
    ...
    if subchoice=='1':
        ...
        if customer:
            ...
            print("\nLooks like this customer has no rentals right now.")

    elif subchoice=='4':
        print_new_line()
        print(f"Awesome! Let's find out who checked-out a video.")
        video_id=input("Please provide the video id: ")
        video = video_store.get_rentals_by_video(video_id=video_id)
        if video and len(video) > 0:
            print("\nHere are the persons that have checked-out this video:")
            for rental in video:
                print(f"Name: {rental['name']}.  Due-date: {rental['due_date'][:16]}")
        else:
            print("\nLooks like no person has cheked-out this video!")
        ...
        subplay=False
        print_spaces()
        print("== Thank you so much for visiting our RENTALS information. We hope to see you soon! ==\n")
        print_spaces()
        print_new_line()
        list_models()
    return subplay
    ...
    while play==True:
        ...
        if choice=='1':
            ...
            while subplay==True:
                ...
                subplay = customer_menu_options(subchoice, video_store)
            ...
            while subplay==True:
                subchoice = make_choice(options, video_store)
                subplay = video_menu_options(subchoice, video_store)
            ...
            while subplay==True:
                subchoice = make_choice(options, video_store)
                subplay = rentals_menu_options(subchoice, video_store)
