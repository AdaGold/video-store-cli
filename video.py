...
class Video:
    ...
    def delete_video(self, id=None, name=None):
        for video in self.list_videos():
            if name:
                if video["title"] == title:
                    id = video["id"]
        ...
        if self.selected_video == None:
            return "Cound not find video by that name or id"
    ...
    def get_video_rentals(self):
        response = requests.get(self.url+f"/videos/{self.selected_video['id']}/rentals")
        self.selected_video = response.json()["rental"]
        return response.json()

    def print_selected(self):
        if self.selected_video:
            print(f"Task with id {self.selected_video['id']} is currently selected\n")
